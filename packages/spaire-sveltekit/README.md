...
`pnpm install @spaire/sveltekit zod`
...
One can either resolve the Spaire Customer ID (by implementing `getCustomerId`) or the external customer ID (by implementing `getExternalCustomerId`). A common example is to resolve the customer by session cookie or JWT token.
